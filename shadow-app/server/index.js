...
function normalizeUkdocsPrintCollection(collection) {
  ...
  const normalized = {
    ...
    documents: {
      ...
      temp_phyto_xml_files: normalizeUkdocsPrintDocumentList(collection?.documents?.temp_phyto_xml_files),
...
// reference-code-level DC/DCS/DCO/CCTAG/PAL/VK data. A "Code" here is a
// specific route/reference (many per carrier, e.g. 4CM140/4CM284/... all
// under carrier "Breewel"), not a customer in its own right, so this is
// stored in its own fust_reference_actions table rather than merged into
// fust_actions -- a Code has no CMR/Fustbon/confirmation workflow. Carrier
// 1/2 are matched against the same Data-tab lookup the Excel importer uses
// (resolveFustImportMeta, unchanged) since the carrier names are shared
// between both sources.
//
// DC-Planning is a same-day forecast; DC-Actual is the confirmed count that
// only gets filled in once a day closes out (both null all day otherwise,
// confirmed against a real sample response). Both are stored as their own
// columns for reference, but nothing here is treated as authoritative
// until the source actually fills it in -- storing null-heavy rows now
// (ahead of the 1 October go-live) is deliberate: it surfaces any Carrier
// 1/2 name that fails to match a Data-tab customer well before the
// deadline, instead of on the day.
const FUST_REFERENCE_IMPORT_WINDOW_DAYS = 2;
...
function fustApiFlagValue(row, field) {
  const value = row?.[field];
  return typeof value === "boolean" ? value : null;
}

// Resolves a reference-code row's carrier via the same Data-tab lookup
// (resolveFustImportMeta, unchanged) the Excel importer uses for carrier
// names -- "Carrier 1"/"Carrier 2" here are the same names as that
// importer's own carrier1_name/carrier2_name.
function mapFustApiRow(row, date, metaRecords) {
  ...
  return {
    ...
    country,
    code,
    group_with: String(row?.["Group with"] || "").trim(),
    carrier1_name: carrier1Name,
    carrier2_name: carrier2Name,
    matched_customer_name: resolvedMeta.matched_by ? resolvedMeta.customer_name : "",
    matched_connect_name: resolvedMeta.matched_by ? resolvedMeta.connect_name : "",
    matched_customer_code: resolvedMeta.matched_by ? resolvedMeta.customer_code : "",
    matched_by: resolvedMeta.matched_by,
    hours: String(row?.Hours || "").trim(),
    flo: fustApiFlagValue(row, "Flo"),
    pla: fustApiFlagValue(row, "Pla"),
    acc: fustApiFlagValue(row, "Acc"),
    all_flag: fustApiFlagValue(row, "All"),
    boxes: fustApiMetricValue(row, "Boxes"),
    dc_planning: fustApiMetricValue(row, "DC-Planning"),
    dc_actual: fustApiMetricValue(row, "DC-Actual"),
  ...
  for (let offset = 0; offset < FUST_REFERENCE_IMPORT_WINDOW_DAYS; offset += 1) {
    ...
    try {
      ...
      for (const row of rows) {
        summary.checked += 1;
        try {
          const mapped = mapFustApiRow(row, date, metaRecords);
          if (!mapped.matched_by) {
            summary.unmatched += 1;
...
function sanitizeAuditEntryName(name) {
  return String(name || "")
    .replace(/[\\/:*?"<>|]/g, "-")
    .replace(/\s+/g, " ")
    .trim() || "unnamed";
...
// flowers/plants, confirmation of exit). Each entry carries the "kind"/index
// the existing per-document download route
// (/api/ukdocs-print/collections/:id/documents/:kind/:index) expects, so the
// manifest can point straight at files already served elsewhere instead of
// duplicating file-serving logic. Factuur PDFs live separately, keyed by
// shipment id in finance_audit_invoice_documents, and are added by the caller.
function collectUkdocsPrintDocumentsForShipment(collection) {
  const documents = collection?.documents || {};
  const entries = [];
  (documents.phyto_files || []).forEach((document, index) => entries.push({ document, kind: "phyto", index }));
  if (documents.export_extra?.storage_name) {
    entries.push({ document: documents.export_extra, kind: "export_extra", index: 0 });
  }
  (documents.generated_files || []).forEach((document, index) => entries.push({ document, kind: "generated", index }));
  if (documents.ipaffs_file?.storage_name) {
    entries.push({ document: documents.ipaffs_file, kind: "ipaffs_file", index: 0 });
  }
  if (documents.ipaffs_plants_file?.storage_name) {
    entries.push({ document: documents.ipaffs_plants_file, kind: "ipaffs_plants_file", index: 0 });
  }
  (documents.exit_confirmation_files || []).forEach((document, index) => entries.push({ document, kind: "exit_confirmation", index }));
  return entries;
}
...
function matchInkoopVeilingLines(erpRows, invoices, supplierMap = {}, supplierNameMap = {}, manualLinks = []) {
  ...
  // with the user, so these are never matched against ERP rows, just
...
function normalizeInkoopSupplierEntryMap(source) {
  const result = {};
  if (source && typeof source === "object") {
    for (const [key, info] of Object.entries(source)) {
      const normalizedKey = normalizeUkdocsText(key);
      if (!normalizedKey || !info?.code) {
        continue;
      }
...
const INKOOP_PRODUCT_GROUP_LABELS = { B: "Bloemen", P: "Planten", A: "Accessoires" };

function inkoopDefaultWindowRange(days) {
  const today = localDateIso();
  return { from: addDaysToIsoDate(today, -Math.abs(Number(days) || 0)), to: addDaysToIsoDate(today, 1) };
}

function inkoopProductGroupLabel(code) {
  const key = normalizeUkdocsText(code).toUpperCase();
  if (!key) {
    return "";
  }
  return INKOOP_PRODUCT_GROUP_LABELS[key] || key;
}

function inkoopCompanyLabel(row) {
  const number = normalizeUkdocsText(row?.company_number);
  const name = normalizeUkdocsText(row?.company_name);
  if (!number && !name) {
    return "";
  }
...
function flattenInkoopInvoiceLines(invoices, sourceFileName, fallbackDate) {
  const flattened = [];
  for (const invoice of Array.isArray(invoices) ? invoices : []) {
    ...
    // buying entities this invoice was billed to -- see server/db.js's
    // inkoop_invoice_lines migration comment for how this was verified.
    const companyNumber = normalizeUkdocsText(invoice?.supplier_number);
    const lines = Array.isArray(invoice?.lines) ? invoice.lines : [];
    lines.forEach((line, index) => {
      // Prefer the real date parsed out of the invoice XML (line_date, set
      // by inkoop_veiling_worker.py from LineDateTime/IssueDateTime) over
      // the date manually typed into the upload form -- only falls back to
      // that when a subject genuinely couldn't be parsed for a date.
      const parsedDate = normalizeUkdocsText(line?.line_date).slice(0, 10);
      flattened.push({
        id: `${invoiceKey}#${index}`,
...
function groupInkoopInvoiceLinesByInvoice(flatLines) {
  ...
  for (const line of Array.isArray(flatLines) ? flatLines : []) {
    ...
    byInvoice.get(key).lines.push({
      // The ledger's stable per-line id -- needed so the follow-up queue
      // can attach a persistent status to a specific "only in invoice" gap
      // across recomputes.
      id: line?.id || "",
      ...
      // The line's real date (see flattenInkoopInvoiceLines) -- now that
      // results span a rolling window instead of one day's pair, every line
...
// (matched/mismatch/only-in-invoice/fee/embalage lines) is narrowed to that
// company. "only in ERP" gaps have no company field at all -- the ERP side
// has no such concept -- so they always show in full with the caller
// responsible for explaining why, rather than being silently hidden or
// wrongly attributed to whichever company happens to be selected.
function filterInkoopResultsByCompany(matched, companyNumber) {
  const normalized = normalizeUkdocsText(companyNumber);
  if (!normalized) {
    return matched;
  }
  const keep = (row) => normalizeUkdocsText(row?.company_number) === normalized;
  return {
    matched_ok: (matched?.matched_ok || []).filter(keep),
    matched_mismatch: (matched?.matched_mismatch || []).filter(keep),
    only_in_invoice: (matched?.only_in_invoice || []).filter(keep),
    only_in_erp: matched?.only_in_erp || [],
    supplier_not_linked: (matched?.supplier_not_linked || []).filter(keep),
    ambiguous_matches: (matched?.ambiguous_matches || []).filter(keep),
    fee_lines: (matched?.fee_lines || []).filter(keep),
    embalage_lines: (matched?.embalage_lines || []).filter(keep),
  };
}
...
function inkoopDateKeyFromErpAnchor(row) {
  const value = row?.erp_row?.date ?? row?.date;
  return String(value || "").slice(0, 10);
}

function inkoopDateKeyFromInvoiceAnchor(row) {
  return String(row?.invoice_date || "").slice(0, 10);
}

function inkoopEnsureDateBucket(buckets, dateKey) {
  if (!dateKey) {
    return null;
  }
...
function bucketInkoopResultsByDate(matched) {
  ...
  for (const row of matched?.embalage_lines || []) {
    const bucket = inkoopEnsureDateBucket(buckets, inkoopDateKeyFromInvoiceAnchor(row));
    if (!bucket) continue;
    bucket.embalage_value += inkoopInvoiceLineTotal(row);
  }

  return buckets;
}

function inkoopDateBucketsToSortedArray(buckets) {
  return [...buckets.values()].sort((left, right) => (left.date < right.date ? 1 : -1));
}

function inkoopWeekStartIso(dateIso) {
  const parsed = new Date(`${String(dateIso || "").slice(0, 10)}T00:00:00`);
  if (Number.isNaN(parsed.getTime())) {
    return "";
  }
  const isoDay = parsed.getDay() || 7;
  parsed.setDate(parsed.getDate() - (isoDay - 1));
  return `${parsed.getFullYear()}-${String(parsed.getMonth() + 1).padStart(2, "0")}-${String(parsed.getDate()).padStart(2, "0")}`;
}

function inkoopBucketDayReportsByWeek(dayBuckets) {
  const weeks = new Map();
  for (const day of dayBuckets) {
    const weekStart = inkoopWeekStartIso(day.date);
    if (!weekStart) {
      continue;
    }
    ...
    week.gap_value += day.gap_value;
    week.embalage_value += day.embalage_value;
    week.product_count += day.product_count;
  }
  return [...weeks.values()].sort((left, right) => (left.week_start < right.week_start ? 1 : -1));
}

function buildInkoopEmbalageSummary(matched) {
  const lines = matched?.embalage_lines || [];
  const totalValue = lines.reduce((sum, row) => sum + inkoopInvoiceLineTotal(row), 0);
  const byDay = inkoopDateBucketsToSortedArray(bucketInkoopResultsByDate({ embalage_lines: lines }))
    .map((bucket) => ({ date: bucket.date, value: bucket.embalage_value }));
  const byCompanyMap = new Map();
  for (const row of lines) {
    const label = inkoopCompanyLabel(row) || "Unknown";
    const key = label.toLowerCase();
    if (!byCompanyMap.has(key)) {
      byCompanyMap.set(key, { key, label, value: 0 });
    }
    byCompanyMap.get(key).value += inkoopInvoiceLineTotal(row);
  }
  const byCompany = [...byCompanyMap.values()].sort((left, right) => right.value - left.value);
...
// inkoop_veiling_worker.py's find_supplier_info); AVC and product group (T:
// B/P/A) only have an identity on the ERP side. An "only in ERP" gap has no
// invoice line yet, so it can only ever be attributed to an AVC/product
// group/never a grower or company; an "only in invoice" gap has no ERP row
// yet, so the reverse. Mismatches have both sides and count toward every
// breakdown.
function buildInkoopDashboardSummary(matched) {
  ...
  for (const row of matched?.only_in_erp || []) {
    ...
    bump(byProductGroup, inkoopProductGroupLabel(row.t), 1, inkoopErpRowTotal(row), 0);
  }
  for (const row of matched?.only_in_invoice || []) {
...
// Stable ids for the follow-up queue -- must survive a recompute (the same
// mismatch/gap always derives the same id) so a status set on one persists
// across every future page load until the underlying data actually changes.
function inkoopIssueId(issueType, row) {
...
function mergeUkdocsPrintCollectionPair(keeper, duplicate) {
  ...
  return normalizeUkdocsPrintCollection({
    ...
    documents: {
      ...
      temp_phyto_xml_files: mergeUkdocsPrintDocumentLists(keeper?.documents?.temp_phyto_xml_files, duplicate?.documents?.temp_phyto_xml_files),
...
function ukdocsPrintCollectionMergeScore(collection) {
  ...
  return (
    ...
    + ((documents.temp_phyto_xml_files || []).length * 3)
...
async function deleteUkdocsPrintCollectionFiles(collection) {
  ...
  if (Array.isArray(collection?.documents?.temp_phyto_xml_files)) {
    documents.push(...collection.documents.temp_phyto_xml_files);
  }
...
function buildUkdocsCsiDomainProducts(baseProducts, sourceRows, domain) {
  ...
  return (Array.isArray(baseProducts) ? baseProducts : [])
    ...
    .map((item) => {
      ...
      const matchingRows = (Array.isArray(sourceRows) ? sourceRows : []).filter((row) => getUkdocsCsiComparisonGroup(row) === productName);
...
function getUkdocsCsiTempPhytoParsedSourceDocuments(collection) {
  const flowerPdfFiles = collection?.documents?.temp_phyto_files || [];
  const flowerXmlFiles = collection?.documents?.temp_phyto_xml_files || [];
  const maxFlowerFiles = Math.max(flowerPdfFiles.length, flowerXmlFiles.length);
  const tempPhytoFiles = [];
  for (let index = 0; index < maxFlowerFiles; index += 1) {
    const xmlDocument = flowerXmlFiles[index] || null;
    const pdfDocument = flowerPdfFiles[index] || null;
    const activeDocument = xmlDocument?.storage_name ? xmlDocument : pdfDocument?.storage_name ? pdfDocument : null;
    if (!activeDocument) {
      continue;
    }
    tempPhytoFiles.push({
...
function buildUkdocsCsiDeterministicReport(collection, extractedDocuments) {
  ...
  const rawTempPhytoDocs = (() => {
    ...
    const extractedTempDocs = documents.filter((document) => document?.kind === "temp_phyto" || document?.kind === "temp_phyto_xml" || document?.kind === "temp_phyto_plants_file" || document?.kind === "temp_phyto_plants_xml_file");
...
async function handleApi(req, res, url) {
  ...
  if (url.pathname.startsWith("/api/ukdocs-print/collections/") && url.pathname.endsWith("/upload") && req.method === "POST") {
    ...
    if (kind === "temp_phyto_xml" && hasUkdocsPrintDocumentWithName(existingCollection.documents?.temp_phyto_xml_files, originalName)) {
  ...
  if (url.pathname.startsWith("/api/ukdocs-print/collections/") && url.pathname.includes("/documents/") && req.method === "DELETE") {
    ...
    if (kind === "phyto") {
      ...
      const tempPhytoXmlFiles = [...(existingCollection.documents?.temp_phyto_xml_files || [])];
      removedDocument = tempPhytoXmlFiles[documentIndex] || null;
      if (!removedDocument) {
        sendJson(res, 404, { error: "UKdocs Print document not found" });
        return;
      }
      tempPhytoXmlFiles.splice(documentIndex, 1);
      updatedDocuments = { ...updatedDocuments, temp_phyto_xml_files: tempPhytoXmlFiles };
  ...
  // manifest gets written to a picked folder via the File System Access API.
  // Each file just points at the same per-document routes already used
  // elsewhere (no duplicate file-serving logic here).
  if (url.pathname === "/api/finance-audit/save-audit-manifest") {
    if (!requirePermission(res, requestUser, PERMISSIONS.UKDOCS_VIEW)) {
      return;
    }
    ...
    for (const shipment of auditedShipments) {
      ...
      const files = [];
      for (const { document, kind, index } of collectUkdocsPrintDocumentsForShipment(collection)) {
        const storagePath = ukdocsPrintDocumentPath(document);
        if (!storagePath || !existsSync(storagePath)) {
          continue;
        }
        files.push({
          name: sanitizeAuditEntryName(document.original_name || document.storage_name),
          url: `/api/ukdocs-print/collections/${encodeURIComponent(collection.id)}/documents/${kind}/${index}`,
        });
      }
      ...
      for (const doc of factuurDocs) {
        ...
        files.push({
          name: sanitizeAuditEntryName(`factuur-${doc.category}-${doc.file_name || doc.storage_name}`),
          url: `/api/finance-audit/invoice-documents/${encodeURIComponent(doc.id)}/file`,
        });
      }
  ...
  // Distinct (company_number, company_name) pairs seen on any uploaded
  // invoice line -- populates the page-level company filter dropdown.
  if (url.pathname === "/api/inkoop/companies" && req.method === "GET") {
    if (!requirePermission(res, requestUser, PERMISSIONS.INKOOP_VIEW)) {
      return;
    }
    const companies = await getInkoopCompanies();
  ...
  // Aggregates the live windowed reconciliation by grower, AVC, company, and
  // product group (B/P/A) for the Inkoop Controle dashboard tab, plus a day
  // report / week report and a dedicated (never matched) Emballage summary
  // -- all pure aggregation of what matchInkoopVeilingLines already
  // produces, no new matching logic.
  if (url.pathname === "/api/inkoop/dashboard/summary" && req.method === "GET") {
    ...
    try {
      ...
      sendJson(res, 200, {
        ...
        embalage_summary: embalageSummary,
  ...
  // Month grid for the Calendar tab -- matches over the month's range plus a
  // 14-day buffer on each side (so a cross-day mismatch pair right at the
  // edge of the month still resolves correctly), then buckets by date and
  // trims to just the requested month.
  if (url.pathname === "/api/inkoop/calendar" && req.method === "GET") {
    if (!requirePermission(res, requestUser, PERMISSIONS.INKOOP_VIEW)) {
      return;
    }
  ...
  // Click-through detail for one calendar day -- same buffered-match
  // approach as the calendar, but returns the full line-level result
  // filtered to that single date, reusing the exact same result shape (and
  // therefore the exact same table markup) as Compare & Reconcile.
  if (url.pathname === "/api/inkoop/day-detail" && req.method === "GET") {
    if (!requirePermission(res, requestUser, PERMISSIONS.INKOOP_VIEW)) {
      return;
    }
    ...
    try {
      ...
      const result = {
        ...
        embalage_lines: companyFiltered.embalage_lines.filter((row) => onDate(row, "invoice")),
      };
      sendJson(res, 200, { date, result });
    } catch (error) {
      sendJson(res, 400, { error: error instanceof Error ? error.message : String(error) });
    }
    return;
  ...
  // Follow-up queue: genuine financial discrepancies (mismatches, gaps on
  // either side) worth someone physically going to check. Supplier-not-
  // linked and ambiguous matches keep their own inline-resolve UI on the
  // Compare tab instead -- nothing to "investigate" there beyond that.
  if (url.pathname === "/api/inkoop/issues" && req.method === "GET") {
    if (!requirePermission(res, requestUser, PERMISSIONS.INKOOP_VIEW)) {
      return;
    }
    try {
      ...
      const merged = issues.map((issue) => {
        const stored = statusById.get(issue.id);
        return {
          ...issue,
      ...
      const filtered = showResolved ? merged : merged.filter((issue) => issue.status !== "resolved");
      filtered.sort((left, right) => right.value - left.value);
      sendJson(res, 200, { issues: filtered });
    } catch (error) {
      sendJson(res, 400, { error: error instanceof Error ? error.message : String(error) });
    }
    return;
  ...
  if (url.pathname.startsWith("/api/inkoop/issues/") && req.method === "POST") {
    if (!requirePermission(res, requestUser, PERMISSIONS.INKOOP_VIEW)) {
      return;
    }
    const issueId = decodeURIComponent(url.pathname.slice("/api/inkoop/issues/".length));
    if (!issueId) {
      sendJson(res, 400, { error: "Missing issue id" });
      return;
    }
    const body = await readRequestJson(req);
    const status = ["open", "investigating", "resolved"].includes(body?.status) ? body.status : "open";
    try {
      await saveInkoopIssueStatus({
        id: issueId,
        issue_type: String(issueId.split(":")[0] || ""),
        status,
